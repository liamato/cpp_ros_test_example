...
TEST(TestExampleNodeNextSrv, next_service) {
    ros::NodeHandle nh("example_node");
    auto timeout = ros::Duration().fromSec(1);
    ros::ServiceClient next = nh.serviceClient<std_srvs::Trigger>("next");
...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "example_node_next_srv", ros::init_options::AnonymousName);
